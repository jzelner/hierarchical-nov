...
camp_avg <- 1.0-exp(-100*exp(zz$log_beta_mu)*zz$zeta)
camp_ci <- quantile(camp_avg, probs = c(0.025, 0.975))
```

On a per-capita basis, the rate of within-camp transmission is approximately `r sprintf("%0.0f",zeta[,"median"])` times greater than the per-capita rate of transmission across *between-camp* contacts (95% CI = `r sprintf("%0.0f, %0.0f", zeta[,"low_ci"], zeta[,"high_ci"])`).

The average betwen-camp probability of transmission per 100 contacts is `r sprintf("%0.2f", 1.0-exp(-100*exp(beta[,"median"])))` (95% CI = `r sprintf("%0.2f, %0.2f", 1.0-exp(-100*exp(beta[,"low_ci"])), 1.0-exp(-100*exp(beta[,"high_ci"])))`).

The average *within-camp* probability of transmission per 100 contacts is `r sprintf("%0.2f", median(camp_avg))`. (95% CI = `r sprintf("%0.2f, %0.2f", camp_ci[1], camp_ci[2])`)
