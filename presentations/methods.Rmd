...
Each case is assumed to have a *per-capita* infectiousness $\beta_i$ drawn from a gamma distribution with shape parameter $\beta_{k}$ and a scale parameter $\beta_{\theta}$. So, $\beta_i \sim Gamma(\beta_{k}, \beta_{\theta})$, and mean value $\beta_{\mu} = \beta_{k} * \beta_{\theta}$.  Because within each camp multiple cases may become infectious on a given day, the values of $\beta_i$ are not individually identifiable. However, we can take advantage of the fact that the sum of $n$ Gamma-distributed variables is Gamma distributed with shape parameter $\beta_{k}*n$ to sample values $\beta_{jt}$ indicating the daily average per-capita infectiousness across all individuals who became infectious in camp $j$ on day $t$. So:

\begin{gather*}
\beta_{jt} = \sum_{i=1}^{y_{jt}} \beta_{i} \\ 
\beta_{i} \sim Gamma(\beta_{k}y_{jt}, \beta_{\theta})
\end{gather*}
